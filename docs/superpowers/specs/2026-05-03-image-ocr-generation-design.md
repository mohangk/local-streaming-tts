...
- Append newly uploaded images to the current unlinked OCR draft instead of creating a new draft when an active draft is open.
- Provide an explicit warning-colored `Clear images` action for starting a fresh OCR draft.
...
4. If no unlinked OCR draft is active, the frontend submits the selected images to create a new OCR draft.
5. If an unlinked OCR draft is active, the frontend submits the selected images to append them to that draft instead of creating another draft.
6. Backend stores each original image, runs OCR for each image, and stores text or failure details per image.
7. Frontend shows a draft review view with one editable combined text area, followed by numbered source thumbnails.
8. User edits or confirms the combined text, and may retry failed image OCR or remove incorrect images before generation.
9. If the user wants to start over instead of appending, they use `Clear images`.
10. User taps Generate audio.
11. Backend creates a normal generation from the draft-level `combined_text` and links it to the OCR draft.
12. Existing streamed playback, history, audio caching, and progress tracking take over.
...
Appending new images to an active unlinked draft preserves the current textarea `combined_text`, including edits that have not yet been generated into audio. Successful OCR text from the newly appended images is appended to that current `combined_text` in upload order, separated by blank lines. Failed appended images remain in the thumbnail list with errors and do not add text until retried successfully.
...
Appending images to an unlinked draft inserts new child image rows after the current maximum `position`. The appended images keep the same storage layout under the existing draft directory.
...
- `POST /api/ocr-drafts/{draft_id}/images`
  - Multipart upload with one or more repeated `image` files plus `language`.
  - Accepts optional `combined_text` so the frontend can preserve current textarea edits while appending.
  - Validates MIME type and size for every file.
...
- Uploading more images while an unlinked draft is active appends those images to the active draft.
...
- Show a warning-colored yellow `Clear images` button when an unlinked draft is active.
- `Clear images` confirms with the user, deletes the active unlinked draft and stored source images through the app deletion flow, clears active review state, and leaves Image mode ready for a fresh upload.
...
- Storage:
  ...
  - Appending images to an existing unlinked OCR draft assigns positions after existing images.
  - Appending successful OCR text preserves existing `combined_text` and adds new text at the end.
...
- API:
  ...
  - `POST /api/ocr-drafts/{draft_id}/images` appends uploaded images to an unlinked draft and updates `combined_text`.
  - Append endpoint rejects linked drafts.
...
- Frontend:
  ...
  - JavaScript appends uploads to the active unlinked OCR draft instead of creating a new draft.
  - JavaScript renders one combined OCR textarea and active source thumbnails.
  - JavaScript exposes a yellow `Clear images` action for deleting the current unlinked draft and starting fresh.
